...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('saveLocalBtn')?.onclick; // géré par onclick HTML
...
// Sérialise un événement FullCalendar en objet JSON fidèle
// — dateToLocalStr pour allDay (évite le décalage UTC)
// — synchronise extendedProps.color avec backgroundColor
function serializeEvent(ev) {
  // Pour allDay : extraire la date en LOCAL (getFullYear/Month/Date)
  ...
  const bgColor = ev.backgroundColor || ev.extendedProps?.color || '#5b8af5';
  // Synchroniser color dans extendedProps avec la couleur visuelle réelle
  const props = { ...ev.extendedProps, color: bgColor };
  return {
    id:    ev.id,
    title: ev.title,
    start: ev.allDay ? dateToLocalStr(ev.start) : ev.start?.toISOString(),
    end:   ev.end    ? (ev.allDay ? dateToLocalStr(ev.end) : ev.end.toISOString()) : null,
    allDay: ev.allDay,
    backgroundColor: bgColor,
    borderColor:     ev.borderColor || bgColor,
    textColor:       ev.textColor   || getContrast(bgColor),
    extendedProps:   props
  };
}

// Alias pour l'undo (garde le snapshot en mémoire)
function snapshotEvent(ev) { return serializeEvent(ev); }
...
function startCopy(event) {
  ...
  S.clipboard = {
    ...
    color:         event.backgroundColor || event.extendedProps.color || '#5b8af5',
...
function pasteOnDate(dateStr) {
  ...
  // Ajouter l'événement au calendrier — couleur = celle de l'event copié
  const pasteColor = c.color;  // couleur exacte au moment de la copie
  const pasteTc    = getContrast(pasteColor);
  S.calendar.addEvent({
    id, title: c.title,
    start: startStr, end: endStr, allDay: c.allDay,
    backgroundColor: pasteColor, borderColor: pasteColor, textColor: pasteTc,
    extendedProps: {
      fileId: srcLayer.id, filename: srcLayer.name,
      color: pasteColor, manual: true,
...
function saveEvent() {
  ...
  if (S.editingId) {
    ...
    if (existing) {
      ...
      // Toujours synchroniser la couleur (même si inchangée, pour la cohérence)
      existing.setProp('backgroundColor', color);
      existing.setProp('borderColor',     color);
      existing.setProp('textColor',       getContrast(color));
      existing.setExtendedProp('color',   color);  // toujours synchronisé
...
function saveLocal(key) {
  // Sauvegarde dans le localStorage du navigateur (pas de téléchargement)
  key = key || 'planviz_autosave';
  const events = S.calendar.getEvents().map(serializeEvent);
  const project = {
    version: '2', savedAt: new Date().toISOString(), appName: 'PlanViz',
    layers: S.layers.map(l => ({ id:l.id, name:l.name, color:l.color, visible:l.visible, manual:l.manual||false })),
    events,
...
function listLocalSaves() {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (k && k.startsWith('planviz_')) {
      try {
        const p = JSON.parse(localStorage.getItem(k));
        if (p && p.appName === 'PlanViz') {
          saves.push({ key: k, savedAt: p.savedAt, events: p.events?.length || 0, layers: p.layers?.length || 0 });
        }
      } catch {}
    }
  }
...
function openLocalSaveModal() {
  ...
  const modal = document.getElementById('localSaveModal');
  const list  = document.getElementById('localSaveList');
  const input = document.getElementById('localSaveName');
  input.value = 'Mon planning ' + new Date().toLocaleDateString('fr-FR');

  if (saves.length === 0) {
    list.innerHTML = '<div style="font-size:12px;color:var(--tx3);text-align:center;padding:12px">Aucune sauvegarde locale</div>';
  } else {
    list.innerHTML = saves.map(s => {
      const d = new Date(s.savedAt).toLocaleString('fr-FR');
      const name = s.key.replace('planviz_', '').replace(/_/g,' ');
      const k = s.key;
      return '<div class="local-save-item">' +
        '<div class="lsi-info"><strong>' + name + '</strong><span>' + d + ' — ' + s.events + ' évts</span></div>' +
        '<div class="lsi-actions">' +
        '<button onclick="loadLocalSave(\'' + k + '\')">Ouvrir</button>' +
        '<button class="danger" onclick="deleteLocalSave(\'' + k + '\')">✕</button>' +
        '</div></div>';
    }).join('');
  }
...
function saveToLocal() {
  const name = document.getElementById('localSaveName')?.value.trim() || 'sauvegarde';
  const key = 'planviz_' + name.toLowerCase().replace(/[^a-z0-9]/g,'_').slice(0,30);
  const ok = saveLocal(key);
  if (ok) {
    toast('💾 Sauvegardé localement : "' + name + '"');
    openLocalSaveModal(); // refresh list
  } else {
    toast('⚠ Stockage local plein — utilisez le téléchargement');
  }
}

function loadLocalSave(key) {
  const project = loadLocal(key);
  document.getElementById('localSaveModal').classList.add('hide');
  if (!project) { alert('Sauvegarde introuvable.'); return; }
  // Réutiliser loadProject en passant le projet directement
  applyProjectData(project);
}

function deleteLocalSave(key) {
...
function applyProjectData(project, filename) {
  try {
    ...
    if (S.layers.length > 0 || S.calendar.getEvents().length > 0) {
      if (!confirm('Ouvrir ce projet ?\nCela remplacera le planning actuel.')) return;
    }
    S.calendar.getEvents().forEach(ev => ev.remove());
    S.layers = []; S.allCategories = new Set(); S.activeCategories = new Set();
    S.undoStack = []; renderLayers(); updateBadge(); updateUndoBtn();
    if (project.theme) {
    ...
    project.layers.forEach(l => S.layers.push({id:l.id,name:l.name,color:l.color,visible:l.visible,events:[],manual:l.manual||false}));
    project.events.forEach(evData => {
      const layer = S.layers.find(l => l.id === evData.extendedProps?.fileId);
      S.calendar.addEvent({
        id:evData.id, title:evData.title, start:evData.start, end:evData.end, allDay:evData.allDay,
        backgroundColor:evData.backgroundColor, borderColor:evData.borderColor, textColor:evData.textColor,
    ...
    project.layers.forEach(l => { if (!l.visible) S.calendar.getEvents().filter(e=>e.extendedProps.fileId===l.id).forEach(e=>e.setProp('display','none')); });
    renderLayers(); updateBadge(); renderCategoryChips(); updateDashIfOpen();
    const dates = project.events.map(e=>new Date(e.start)).filter(d=>!isNaN(d)).sort((a,b)=>a-b);
    if (dates.length) S.calendar.gotoDate(dates[0]);
    const src = filename ? '"' + filename + '"' : 'la sauvegarde';
    toast('📂 ' + project.events.length + ' événements chargés depuis ' + src);
