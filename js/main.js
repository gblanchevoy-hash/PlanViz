...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('keydown', e => {
    if (e.key==='Escape') { closeCtx(); if (S.pasteMode) cancelPaste(); if (S.eraserMode) app_cancelEraser(); }
...
function initCalendar() {
  S.calendar = new FullCalendar.Calendar(document.getElementById('calendar'), {
    ...
    views:{ dayGridFourWeek:{ type:'dayGrid', duration:{weeks:13} }},
...
function closeCtxOutside(e) { const m=document.getElementById('ctxMenu'); if(!m.classList.contains('hide')&&!m.contains(e.target)) closeCtx(); }
...
function openEventModal(event=null, prefillDate=null) {
  ...
  if(event){ S.editingId=event.id;
    addDateRow(
      event.allDay ? fcDateToStr(event.start) : dateToStr(event.start),
      event.allDay ? '' : (event.start?.toTimeString().slice(0,5)||'09:00'),
      event.allDay ? fcDateToStr(event.end||event.start) : dateToStr(event.end||event.start),
      event.allDay ? '' : (event.end?.toTimeString().slice(0,5)||'10:00')
    );
...
function openViewModal(event) {
  S.viewMode=true; S.editingId=null; S.dateRowCount=0; setRO(true);
  document.getElementById('mEvtTitle').textContent='📋 Détail';
  document.getElementById('mEvtSub').textContent=event.extendedProps.filename||'';
  document.getElementById('evTitle').value=event.title||'';
  document.getElementById('evCat').value=event.extendedProps.category||'';
  document.getElementById('evRes').value=event.extendedProps.resource||'';
  document.getElementById('evDesc').value=event.extendedProps.description||'';
  document.getElementById('evDelBtn').classList.add('hide');
  document.getElementById('evSaveBtn').textContent='Fermer';
  document.getElementById('evSaveBtn').onclick=closeEventModal;
  document.getElementById('addDateBtn').style.display='none';
  const color=event.extendedProps.color||event.backgroundColor||BASE_PALETTE[0];
  S.selectedColor=color; document.getElementById('evColor').value=color;
  rebuildSwatches(); setColorUI(color);
  document.getElementById('dateRows').innerHTML='';
  addDateRow(
    event.allDay ? fcDateToStr(event.start) : dateToStr(event.start),
    event.allDay ? '' : (event.start?.toTimeString().slice(0,5)||''),
    event.allDay ? fcDateToStr(event.end||event.start) : dateToStr(event.end||event.start),
    event.allDay ? '' : (event.end?.toTimeString().slice(0,5)||'')
  );
  document.querySelectorAll('.dr-fields input').forEach(i=>i.readOnly=true);
  document.getElementById('modalEvent').classList.remove('hide');
}
...
function saveEvent() {
  ...
  const layerName = document.getElementById('evLayerName')?.value.trim() || title;
  ...
  // Si édition : supprimer l'ancien événement de son ancienne couche
  if (S.editingId) {
    const oldEv = S.calendar.getEventById(S.editingId);
    if (oldEv) {
      const oldLayer = S.layers.find(l => l.id === oldEv.extendedProps.fileId);
      if (oldLayer) {
        oldLayer.events = oldLayer.events.filter(e => e.id !== S.editingId);
        if (oldLayer.events.length === 0 && oldLayer.manual) {
          S.layers = S.layers.filter(l => l.id !== oldLayer.id);
        }
      }
      oldEv.remove();
    }
  ...
  // Obtenir ou créer la couche pour ce nom
  const layer = getOrCreateManualLayer(layerName, color);

  // Ajouter les événements
  entries.forEach(({ start, end, allDay }) => {
    const id = 'manual_' + Date.now() + '_' + Math.random().toString(36).substr(2,4);
    const ss = allDay ? start : (start instanceof Date ? start.toISOString() : start);
    const ee = allDay ? end   : (end   instanceof Date ? end.toISOString()   : end);
    S.calendar.addEvent({
  ...
  closeEventModal();
  toast(`${entries.length} événement(s) ajouté(s) dans "${layer.name}"`);
}

function deleteEditEvent(){
  if(!S.editingId||!confirm('Supprimer ?')) return;
  S.calendar.getEventById(S.editingId)?.remove();
  const layer=S.layers.find(l=>l.id==='manual'); if(layer)layer.events=layer.events.filter(e=>e.id!==S.editingId);
  renderLayers(); updateDashIfOpen(); closeEventModal();
}
...
function initFilters(){
  ...
  document.getElementById('clearDatesBtn').onclick=()=>{document.getElementById('fFrom').value='';document.getElementById('fTo').value='';applyFilters();};
}
...
function applyFilters(){
  ...
  S.calendar.getEvents().forEach(ev=>{
    ...
    if(show&&q){const hay=(ev.title+' '+(ev.extendedProps.category||'')+' '+(ev.extendedProps.resource||'')).toLowerCase();show=hay.includes(q);}
...
function updateUndoBtn() {
  ...
  btn.title = S.undoStack.length > 0
    ? `Annuler: ${S.undoStack[S.undoStack.length-1].type === 'delete' ? 'suppression' : 'ajout'} (Ctrl+Z)`
    : 'Rien à annuler';
}
